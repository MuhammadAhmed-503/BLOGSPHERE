...
import { AlignCenter, AlignJustify, AlignLeft, AlignRight, Bold, Code, CornerDownLeft, Heading1, Heading2, Heading3, ImagePlus, Italic, Link2, Loader2, ListOrdered, Pilcrow, Redo2, RemoveFormatting, Strikethrough, Subscript, Unlink, Undo2, Youtube as YoutubeIcon, } from 'lucide-react';
function ToolbarButton({ active, disabled, onClick, label, children, }) {
    return (<button type="button" onClick={onClick} disabled={disabled} aria-label={label} className={`inline-flex h-9 w-9 items-center justify-center rounded-lg border text-sm transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${active ? 'border-primary-500 bg-primary-50 text-primary-700 dark:border-primary-400 dark:bg-primary-950/40 dark:text-primary-300' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800'}`}>
      {children}
    </button>);
}
export default function RichTextEditor({ value, onChange, placeholder, onUploadImage }) {
    ...
    const editor = useEditor({
        ...
        editorProps: {
            attributes: {
                class: 'tiptap prose prose-slate max-w-none min-h-[400px] px-4 py-4 focus:outline-none dark:prose-invert dark:text-slate-100',
            },
        },
    ...
    return (<div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="sticky top-0 z-20 flex max-h-48 flex-wrap items-center gap-2 overflow-y-auto border-b border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/95">
        ...
        <div className="flex flex-wrap items-center gap-1 rounded-lg border border-slate-200 bg-white p-1 dark:border-slate-700 dark:bg-slate-900">
          <div className="relative">
            ...
            {linkPopoverOpen && (<div className="absolute left-0 top-11 z-20 w-72 rounded-xl border border-slate-200 bg-white p-3 shadow-xl dark:border-slate-700 dark:bg-slate-900">
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading || !onUploadImage} className="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800" aria-label="Upload image">
          ...
          <div className="relative">
            ...
            {youtubePopoverOpen && (<div className="absolute left-0 top-11 z-20 w-72 rounded-xl border border-slate-200 bg-white p-3 shadow-xl dark:border-slate-700 dark:bg-slate-900">
      ...
      <div className="relative max-h-[65vh] overflow-y-auto" onDragOver={(event) => event.preventDefault()} onDrop={(event) => void handleDrop(event)}>
        {uploading && (<div className="absolute inset-x-0 top-3 z-10 mx-auto flex w-fit items-center gap-2 rounded-full bg-slate-950 px-3 py-1 text-xs font-medium text-white shadow-lg">
