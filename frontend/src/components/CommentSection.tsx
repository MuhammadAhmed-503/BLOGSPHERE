...
import { createPostComment, fetchPostComments } from '../lib/api';
...
export default function CommentSection({ slug }: CommentSectionProps) {
  ...
  return (
    <section className="mx-auto mt-16 max-w-4xl border-t border-slate-200 px-4 pt-12 dark:border-slate-800 sm:px-6 lg:px-8">
      ...
      <form onSubmit={handleSubmit} className="card mb-8 space-y-3 p-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <input
            ...
            onChange={(event) => setFormData((prev) => ({ ...prev, authorEmail: event.target.value }))}
            required
          />
        </div>
        <textarea
      ...
      {loading ? (
        ...
        <p className="text-slate-600 dark:text-slate-400">No approved comments yet.</p>
      ) : (
