...
import { fetchCategories, fetchPosts, fetchTags } from '../services/api';
import { formatDate } from '../utils';
const defaultLimit = 4;
function buildQuery(params, updates) {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-4">
          <aside className="lg:col-span-1">
            {categories.length > 0 && (<div className="card mb-6 p-6">
                <h3 className="mb-4 font-bold text-slate-900 dark:text-slate-100">Categories</h3>
                <div className="space-y-2">
                  <Link to="/blog" className={`block rounded-lg px-3 py-2 transition-colors ${!category ? 'bg-primary-100 text-primary-700 dark:bg-primary-900 dark:text-primary-300' : 'text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'}`}>
                    All Posts
                  </Link>
                  {categories.map((item) => (<Link key={item.name} to={`/blog${buildQuery(searchParams, { category: item.name, page: null, tag: null })}`} className={`block rounded-lg px-3 py-2 transition-colors ${category === item.name ? 'bg-primary-100 text-primary-700 dark:bg-primary-900 dark:text-primary-300' : 'text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'}`}>
                      {item.name} ({item.count})
                    </Link>))}
                </div>
              </div>)}

            {tags.length > 0 && (<div className="card p-6">
                <h3 className="mb-4 font-bold text-slate-900 dark:text-slate-100">Popular Tags</h3>
                <div className="flex flex-wrap gap-2">
                  {tags.slice(0, 15).map((item) => (<Link key={item.name} to={`/blog${buildQuery(searchParams, { tag: item.name, page: null, category: null })}`} className={`rounded-full px-3 py-1 text-sm transition-colors ${tag === item.name ? 'bg-primary-600 text-white' : 'bg-slate-200 text-slate-700 hover:bg-slate-300 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600'}`}>
                      {item.name}
                    </Link>))}
                </div>
              </div>)}
          ...
          <main className="lg:col-span-3">
            ...
            {error && (<div className="card mb-6 p-6">
                <p className="text-red-600 dark:text-red-400">{error}</p>
              </div>)}

            {(category || tag) && (<div className="mb-6 flex flex-wrap items-center gap-2">
            ...
            {!loading && blogs.length === 0 ? (<div className="card p-12 text-center">
                <p className="text-lg text-slate-600 dark:text-slate-400">No blog posts found.</p>
              </div>) : (<div className="space-y-8">
                {blogs.map((blog) => (<Link key={blog.id} to={`/blog/${blog.slug}`} className="card group flex flex-col overflow-hidden sm:flex-row">
                    ...
                    <div className="flex-grow p-6">
                      <div className="mb-3 flex items-center gap-2">
                        ...
                        {blog.tags.slice(0, 2).map((blogTag) => (<span key={blogTag} className="rounded bg-slate-200 px-2 py-1 text-xs text-slate-700 dark:bg-slate-700 dark:text-slate-300">
                            {blogTag}
                          </span>))}
                      </div>
            ...
            {total > defaultLimit && (<div className="mt-8 flex justify-center gap-2">
                {page > 1 && (<Link to={`/blog${buildQuery(searchParams, { page: page - 1 })}`} className="btn-secondary">
                    Previous
                  </Link>)}
                <span className="px-4 py-2 text-slate-700 dark:text-slate-300">
                  Page {page} of {totalPages}
                </span>
                {page < totalPages && (<Link to={`/blog${buildQuery(searchParams, { page: page + 1 })}`} className="btn-secondary">
                    Next
                  </Link>)}
              </div>)}
          </main>
