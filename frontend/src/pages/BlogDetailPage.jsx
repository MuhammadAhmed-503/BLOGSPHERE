...
export default function BlogDetailPage() {
    ...
    if (loading) {
        return (<div className="min-h-screen py-16 text-center">
    ...
    return (<article className="min-h-screen py-8">
      <div className="mx-auto mb-8 max-w-4xl px-4 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-wrap items-center gap-3">
          <span className="badge text-sm">{blog.category}</span>
          {blog.tags.slice(0, 3).map((tag) => (<span key={tag} className="rounded-full bg-slate-200 px-3 py-1 text-sm dark:bg-slate-700">
              {tag}
            </span>))}
        </div>
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <div className="prose prose-slate max-w-none dark:prose-invert" dangerouslySetInnerHTML={{ __html: htmlContent }}/>
      </div>
      ...
      {relatedBlogs.length > 0 && (<div className="mx-auto mt-16 max-w-4xl border-t border-slate-200 px-4 pt-16 sm:px-6 lg:px-8 dark:border-slate-800">
          <h2 className="mb-8 text-3xl font-bold text-slate-900 dark:text-slate-100">
            Related Posts
          </h2>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {relatedBlogs.map((relatedBlog) => (<Link key={relatedBlog.id} to={`/blog/${relatedBlog.slug}`} className="card group overflow-hidden">
