...
import { deleteAdminBlog, fetchAdminBlogs, setBlogStatus } from '../../lib/adminApi';
...
export default function BlogsPage() {
  ...
  return (
    <div>
      <AdminPageHeader
        title="Blogs"
        subtitle="Review, search, publish, unpublish, and delete blog posts from one control surface."
          ...
          <div className="flex flex-wrap gap-2">
            {(['all', 'published', 'draft'] as BlogFilter[]).map((filter) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${activeFilter === filter ? 'bg-primary-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700'}`}
              >
      ...
      {loading ? (
        <div className="card p-6 text-slate-600 dark:text-slate-400">Loading blogs...</div>
        ...
        <div className="space-y-4">
          {visibleBlogs.map((blog) => {
            ...
            return (
              <div key={blog.id} className="card overflow-hidden p-4 md:p-5">
                <div className="flex flex-col gap-4 md:flex-row">
                  ...
                  <div className="flex flex-wrap items-center gap-2 md:self-start">
                    ...
                    {blog.status === 'published' && (
                      <a href={`/blog/${blog.slug}`} target="_blank" rel="noreferrer" className="btn-secondary inline-flex items-center gap-2">
