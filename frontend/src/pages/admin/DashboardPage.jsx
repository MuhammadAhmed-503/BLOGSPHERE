...
import { fetchAdminBlogs, fetchAdminComments, fetchAdminDashboard, fetchAdminSubscribers } from '../../services/adminApi';
...
export default function DashboardPage() {
    ...
    return (<div>
      <AdminPageHeader title="Dashboard" subtitle="Welcome back. Here is a fast snapshot of the editorial workflow and audience health." actions={<Link to="/admin/create" className="btn-primary inline-flex items-center gap-2"><PlusCircle className="h-4 w-4"/>Create New Blog</Link>}/>

      {loading ? (<div className="card p-6 text-slate-600 dark:text-slate-400">Loading dashboard...</div>) : (<div className="space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="card p-6">
              ...
              {mostViewedBlog ? (<div className="space-y-4">
                ...
                </div>) : (<p className="text-slate-500 dark:text-slate-400">No blog posts are available yet.</p>)}
            </div>

            <div className="card p-6">
              <h2 className="mb-5 text-xl font-bold text-slate-900 dark:text-slate-100">Recent Activity</h2>
              <div className="space-y-4">
                {activities.length === 0 ? (<p className="text-slate-500 dark:text-slate-400">No recent activity found.</p>) : (activities.map((activity) => (<div key={activity.id} className="flex items-start justify-between gap-4 rounded-xl border border-slate-200 p-4 dark:border-slate-800">
                      <div>
