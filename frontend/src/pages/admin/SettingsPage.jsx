...
export default function SettingsPage() {
    ...
    return (<div className="mx-auto max-w-4xl">
      <AdminPageHeader title="Settings" subtitle="Control access, signup rules, and commenting behavior from one place."/>

      {banner && (<div className={`mb-6 rounded-2xl border px-4 py-3 text-sm ${banner.type === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-900/50 dark:bg-emerald-950/30 dark:text-emerald-200' : 'border-red-200 bg-red-50 text-red-800 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-200'}`}>
          {banner.message}
        </div>)}

      {loading ? (<div className="card p-6 text-slate-600 dark:text-slate-400">Loading settings...</div>) : settings ? (<div className="space-y-6">
          <div className="grid gap-4">
            <ToggleSwitch label="Require User Login" description="When enabled, readers must be authenticated before interacting with protected features." checked={Boolean(settings.requireUserLogin)} onChange={() => toggle('requireUserLogin')} disabled={savingKey === 'requireUserLogin'}/>

            <ToggleSwitch label="Allow User Signup" description="Permit new users to create accounts from the public site." checked={Boolean(settings.allowUserSignup)} onChange={() => toggle('allowUserSignup')} disabled={savingKey === 'allowUserSignup'}/>

            <ToggleSwitch label="Allow Anonymous Comments" description="Let visitors post comments without logging in." checked={Boolean(settings.allowAnonymousComments)} onChange={() => toggle('allowAnonymousComments')} disabled={savingKey === 'allowAnonymousComments' || Boolean(settings.requireUserLogin)}/>
          </div>
        ...
        </div>) : (<div className="card p-6 text-slate-600 dark:text-slate-400">Settings could not be loaded.</div>)}
    </div>);
}
