...
import { deleteAdminSubscriber, fetchAdminSubscribers, notifyAdminSubscribers } from '../../services/adminApi';
...
export default function SubscribersPage() {
    ...
    return (<div>
      <AdminPageHeader title="Subscribers" subtitle="Manage newsletter subscribers, remove records, and send broadcast notifications." actions={<button type="button" onClick={handleNotifyAll} disabled={notifying} className="btn-primary inline-flex items-center gap-2"><MailPlus className="h-4 w-4"/>{notifying ? 'Sending...' : 'Notify All'}</button>}/>
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {['all', 'verified', 'unverified'].map((option) => (<button key={option} type="button" onClick={() => setFilter(option)} className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${filter === option ? 'bg-primary-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700'}`}>
            {option === 'all' ? `All (${counts.total})` : option === 'verified' ? `Verified (${counts.verified})` : `Unverified (${counts.pending})`}
          </button>))}
      </div>

      {loading ? (<div className="card p-6 text-slate-600 dark:text-slate-400">Loading subscribers...</div>) : visibleSubscribers.length === 0 ? (<div className="card p-6 text-slate-600 dark:text-slate-400">No subscribers match the current filter.</div>) : (<div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                ...
                return (<tr key={subscriber.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/40">
