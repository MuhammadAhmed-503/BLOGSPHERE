...
import { adminBlogSchema, stripHtmlToText } from '../../services/adminSchemas';
import { createAdminBlog, fetchAdminBlogById, updateAdminBlog, uploadAdminFile, } from '../../services/adminApi';
...
export default function BlogEditorPage({ mode }) {
    ...
    const [formValue, setFormValue] = useState(createEmptyState());
    ...
    const validatePayload = (payload, publish) => {
    ...
    if (initialLoading) {
        return (<div className="card p-6 text-slate-600 dark:text-slate-400">Loading editor...</div>);
    }
    return (<div>
      <div className="mb-6">
      ...
      <BlogEditorForm mode={mode} value={formValue} onChange={setFormValue} onCancel={() => navigate('/admin/blogs')} onSaveDraft={handleSaveDraft} onPublish={handlePublish} onUploadEditorImage={handleUploadEditorImage} onUploadCoverImage={handleUploadCoverImage} loading={saving} publishing={publishing}/>
    </div>);
}
