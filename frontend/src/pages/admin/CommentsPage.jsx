...
import { approveAdminComment, deleteAdminComment, fetchAdminComments } from '../../services/adminApi';
...
export default function CommentsPage() {
    ...
    return (<div>
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {['all', 'pending', 'approved'].map((option) => (<button key={option} type="button" onClick={() => setFilter(option)} className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${filter === option ? 'bg-primary-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700'}`}>
            {option === 'all' ? `All (${counts.total})` : option === 'pending' ? `Pending (${counts.pending})` : `Approved (${counts.approved})`}
          </button>))}
      </div>

      {loading ? (<div className="card p-6 text-slate-600 dark:text-slate-400">Loading comments...</div>) : visibleComments.length === 0 ? (<div className="card p-6 text-slate-600 dark:text-slate-400">No comments found for this filter.</div>) : (<div className="space-y-4">
          {visibleComments.map((comment) => {
                const isBusy = busyId === comment.id;
                return (<div key={comment.id} className={`card border-l-4 p-5 ${comment.status === 'pending' ? 'border-l-amber-400' : 'border-l-emerald-500'}`}>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {comment.status === 'pending' && (<button type="button" onClick={() => handleApprove(comment)} disabled={isBusy} className="btn-primary inline-flex items-center gap-2">
